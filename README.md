...
```bash
conda create python=3.8 -n student_performance
```
...
If for some reason you get an error similar to bellow, is because the DATABASE_URL is configured to `postgres:///student_performance` and because of it the generated `DATABASES` settings are configured to connect on PostgreSQL using the socket mode.
...
LINE 1: CREATE DATABASE student_performance WITH ENCODING 'UTF-8' OWNER "";
                                                             ^
ERROR:  syntax error at or near ";"
LINE 1: GRANT ALL PRIVILEGES ON DATABASE student_performance TO ;
```
...
ERROR:  database "student_performance" already exists
GRANT
```
...
```bash
sudo -u postgres dropdb -U postgres --if-exists student_performance
...
```bash
dropdb --if-exists student_performance
